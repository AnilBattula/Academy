...
public class BookACruiseStep extends TestEngineWeb {
	...
	/**
	 * The user has the access to tall link.
	 *
	...
	public void the_user_has_the_access_to_TallLink() throws Throwable {
		try {
			...
			testCaseFailureReason = "Failed to complete - " + "the user has the access to TallLink";
	...
	public void the_user_is_able_to_book_a_cruise() throws Throwable {
		try {
			...
			actionLib.Click(CommonVariables.CommonDriver.get().findElement(bookCruise.btnAdults), 10,
					"selecting adults");
			actionLib.Click(CommonVariables.CommonDriver.get().findElement(bookCruise.btnChildrens), 10,
					"selecting childrens");
			actionLib.Click(CommonVariables.CommonDriver.get().findElement(bookCruise.selectFromIcon), 10,
					"click Icon");
			actionLib.Click(CommonVariables.CommonDriver.get().findElement(bookCruise.departureFrom(from)), 10,
					"click departure from");

			actionLib.Click(CommonVariables.CommonDriver.get().findElement(bookCruise.destination(to)), 10,
					"click destination");

			actionLib.Click(CommonVariables.CommonDriver.get().findElement(bookCruise.btnBookYourTrip), 10,
					"click on book your trip");
			...
			actionLib.Click(CommonVariables.CommonDriver.get().findElement(bookCruise.btnVechiclle), 10,
					"click on vechicle");
			actionLib.WaitForElementVisible(bookCruise.vechicleIcon, 100000, "clickk on vechicle");
			actionLib.Click(CommonVariables.CommonDriver.get().findElement(bookCruise.vechicleIcon), 10,
					"click on vechicle Icon");
			actionLib.WaitForElementVisible(bookCruise.selectCar, 100000, "clickk on vechicle");
			List<WebElement> noOfVehicle = CommonVariables.CommonDriver.get().findElements(bookCruise.selectCar);
			for (int i = 1; i <= noOfVehicle.size(); i++) {
				if (!noOfVehicle.get(i).getText().contains("SOLD OUT")) {
					actionLib.Click(noOfVehicle.get(i), 10, "Vehicle is Selected");
					break;
				}
			}

			actionLib.Click(CommonVariables.CommonDriver.get().findElement(bookCruise.clickPlusIcon), 30,
					"click on car Icon");
			actionLib.Click(CommonVariables.CommonDriver.get().findElement(bookCruise.clickPlusIcon), 30,
					"click on car Icon");
			actionLib.Click(CommonVariables.CommonDriver.get().findElement(bookCruise.btnBookTrip), 30,
					"click on btnBookTrip");

		} catch (Exception e) {
			testCaseFailureReason = "Failed to complete - " + "the user is able to book a cruise";
			String stackTrace = extentLogs.getStackTraceAsString("Test", testCaseFailureReason, e);
			extentLogs.fail(scenarioName, testCaseFailureReason + "Failed Reason : " + stackTrace);
			testCaseStatus = false;
		}
	...
	public void user_should_be_able_to_navigate_Payment_page() throws Throwable {
		try {
			...
			testCaseFailureReason = "Failed to complete - " + "user should be able to navigate Payment page";
			String stackTrace = extentLogs.getStackTraceAsString("Test", testCaseFailureReason, e);
			extentLogs.fail(scenarioName, testCaseFailureReason + "Failed Reason : " + stackTrace);
			testCaseStatus = false;
		}
